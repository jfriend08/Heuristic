...
public:
  ...
  vector<vector<int> > shuffle(vector<vector<int> > allCity) {
    //try suffline from given list to have better order
    int numTry = 3;
    float curScore = getTotalDist(allCity);

    for(int repeat=0; repeat<numTry; repeat++) {
      int city_idx1 = rand() % allCity.size();
      int city_idx2 = rand() % allCity.size();
      vector<vector<int> > swapCity = swapCities(city_idx1,city_idx2,allCity);
      float swapScore = getTotalDist(swapCity);
      if (swapScore<curScore) {
        allCity = swapCity;
        curScore = swapScore;
      }
    }
  ...
  vector<vector<int> > greedyFindInitPath(vector<vector<int> > allCity) {
    ...
    while(greedyCityOrder.size()< numCity) {
      ...
      // greedyCityOrder = shuffle(greedyCityOrder);
      haveSeen[currentBestNextCity_idx] = true;
      luckyCity_idx = greedyCityOrder[greedyCityOrder.size()-1][0]-1;
    }
    return greedyCityOrder;
  ...
  void findBestRout(vector<vector<int> > allCity, int x_max, int x_min, int y_max, int y_min, int z_max, int z_min){
    ...
    for(int repeat=0; repeat<numGreedyTry; repeat++) {
       vector<vector<int> > curCityOrder = greedyFindInitPath(allCity);
       double curScore = getTotalDist(curCityOrder);
       if(curScore < bestScore) {
        // cout<<"bestScore update: "<<bestScore<<"==>"<<curScore<<endl;
        bestScore = curScore;
        cityOrder = curCityOrder;
       }

    }
    // cout<<"initial score: " << bestScore<<endl;
    while(T > T_min) {
      int city_idx1 = 1, city_idx2 = 1;
      while (city_idx1 == city_idx2) {
        city_idx1 = rand() % allCity.size();
        city_idx2 = rand() % allCity.size();
      }
