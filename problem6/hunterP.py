...
class Hunter(object):
  ...
  def remove_walls(self, cmd):
    ...
    print "remove_walls. self.direction", self.direction
    if self.direction[0] == 0 or self.direction[1] == 0 :
      print "remove_walls. self.direction", self.direction
      for i in xrange(len(self.walls)):
        if self.walls[i]['position'][0] == self.hunter[0] or self.walls[i]['position'][1] == self.hunter[1]:
          cmd['wallIds'].append(self.walls[i]['id'])
  ...
  def move_in_front(self):
    ...
    if len(self.walls) >= self.maxWalls or self.direction[0] == 0 or self.direction[1] == 0:
  ...
  def make_move(self, cmd):
    ...
    self.direction = [int(cmd['hunter'][0]) - int(self.hunter[0]), int(cmd['hunter'][1]) - int(self.hunter[1])]
    self.hunter = cmd['hunter']
    self.prey = cmd['prey']
    self.time = cmd['time']

    print "cmd['hunter']", cmd['hunter']
    print "self.hunter", self.hunter
